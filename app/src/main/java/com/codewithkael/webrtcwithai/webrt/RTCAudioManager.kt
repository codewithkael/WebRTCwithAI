@file:Suppress("DEPRECATION", "UNCHECKED_CAST") @file:SuppressLint(
...
class RTCAudioManager(context: Context) {
    ...
    private inner class WiredHeadsetReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent) {
            ...
            Log.d(
                TAG,
                "WiredHeadsetReceiver.onReceive" + ": " + "a=" + intent.action.toString() + ", s=" + (if (state == stateUnplugged) "unplugged" else "plugged").toString() + ", m=" + (if (microphone == hasMic) "mic" else "no mic").toString() + ", n=" + name.toString() + ", sb=" + isInitialStickyBroadcast
            )
...
//        }
    ...
    fun stop() {
        ...
        if (amState != AudioManagerState.RUNNING) {
            Log.e(
                TAG, "Trying to stop AudioManager in incorrect state: $amState"
            )
            return
        }
    ...
    fun selectAudioDevice(device: AudioDevice) {
        ...
        if (!audioDevices.contains(device)) {
            Log.e(
                TAG, "Can not select $device from available $audioDevices"
            )
        }
    ...
    fun updateAudioDeviceState() {
        ThreadUtils.checkIsOnMainThread()
        Log.d(
            TAG, ("--- updateAudioDeviceState: wired headset=$hasWiredHeadset")
        )
        Log.d(
            TAG,
